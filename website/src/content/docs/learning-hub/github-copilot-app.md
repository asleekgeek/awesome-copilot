...
*(v1.1.20+)* Generated Markdown artifacts—like research reports or plans a session produces—now open in the **Files tab** alongside your repository files, with a switcher to move between generated artifacts and added file/folder sources. A **promote** action lets you turn a useful artifact into a real file in the repository instead of copy-pasting its contents by hand. A source switcher in the Files tab *(v1.1.17+)* lets you browse these sources without leaving the tab.

### Chats Pill and Side Conversations

A **Chats pill** above the composer *(v1.1.18+)* shows unread and input-required status for side conversations, with quick navigation to jump straight to the chat that needs your attention—useful when you have several side discussions going alongside the main session.

### Disambiguating Agents from Multiple Plugins

*(v1.1.20+)* If two installed plugins each contribute a custom agent with the same display name, the agent picker now shows which plugin each one belongs to, so you can tell them apart before starting a session.

> **Naming update (v1.1.20+)**: The "Start from scratch" option in session creation menus and project pickers is now labeled **"Chat"**.
