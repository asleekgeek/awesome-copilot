...
> **Namespaced and ignored skill directories (v1.0.86+)**: Skill discovery now supports namespaced custom skill directories, so skills bundled by different plugins or sources with overlapping names no longer collide, and directories explicitly marked as ignored are skipped during discovery instead of being loaded.
