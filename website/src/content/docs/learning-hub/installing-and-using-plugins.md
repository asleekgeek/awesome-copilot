...
**Non-interactive plugin management (v1.0.84+)**: The cross-kind `copilot plugins` command has been split into kind-specific commands so scripts and CI can manage each component type directly:

```bash
# Enable or disable a plugin, MCP server, or skill without opening the dashboard
...
# List instructions and LSP servers directly (previously copilot plugins list --kind instruction|lsp)
copilot instruction list
copilot lsp list

# Add JSON output to list/browse commands for scripting
copilot plugin list --json
copilot plugin marketplace list --json
copilot plugin marketplace browse --json
```

> **Breaking change (v1.0.84+)**: `copilot plugins install --skill [--scope project]` is replaced by `copilot skill add [--project]` — the `--scope` spelling is gone. The cross-kind `--kind`, `--scope`, `--mcp`, and `--skill` flags are removed from `copilot plugins`; use the dedicated `copilot mcp` and `copilot skill` commands instead. `copilot plugins list --json` now returns a flat array of plugins rather than the old `{ plugins, errors }` object, and plain `copilot plugins list` is now an alias for `copilot plugin list` that reports only plugins (not MCP servers, skills, instructions, or LSP servers).
