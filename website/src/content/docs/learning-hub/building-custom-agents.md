...
**include-custom-instructions** *(v1.0.86+)*: By default, custom agents run without repository instruction files like `AGENTS.md`, `copilot-instructions.md`, or `CLAUDE.md`, so their behavior is defined entirely by their own frontmatter and prompt. Set `include-custom-instructions: true` to have the agent also pick up those repository-level instruction files, layering its specialized persona on top of your team's general coding conventions:
