...
**Fallback model lists** *(v1.0.83+)*: `model` can list several models instead of a single one. Copilot tries them in order and uses the first one available to you, which is useful when a preferred model is occasionally rate-limited or not enabled for your account:
...
Add `model-policy: required` alongside a model list to keep any in-session model changes restricted to that list, preventing the agent from being switched to a model outside its approved set.
