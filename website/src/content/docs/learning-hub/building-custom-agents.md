...
**model** *(list support, v1.0.83+)*: `model` can now list several models, tried in order until one is available to you (for example, if your first choice is temporarily rate-limited or not enabled for your plan). Pair this with **`model-policy: required`** to keep the agent locked to that list — preventing a user from manually switching the agent to an unlisted model mid-session:

```yaml
---
name: 'Security Reviewer'
description: 'Expert security auditor for OWASP vulnerabilities'
model: ['Claude Sonnet 5', 'Claude Opus 5', 'GPT-5.6']
