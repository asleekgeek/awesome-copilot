...
**include-custom-instructions** *(v1.0.86+)*: By default, a custom agent's behavior is defined entirely by its own frontmatter and instructions—it does not automatically inherit the repository's general instruction files. Set `include-custom-instructions: true` to have the agent also load repository instruction files such as `AGENTS.md`, `.github/copilot-instructions.md`, or `CLAUDE.md` alongside its own persona. This is useful when an agent should follow both its specialized guardrails and the team's overall coding conventions:
