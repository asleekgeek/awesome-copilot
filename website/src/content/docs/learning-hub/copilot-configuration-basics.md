...
**Sandbox network isolation** *(v1.0.83+, breaking change)*: On macOS and Linux, sandboxed commands can no longer reach services running on your own machine by default. On macOS this also blocks a server the sandboxed command itself starts on `127.0.0.1`, so test suites that bind a local port will fail until you enable **Allow local network** in `/sandbox`. Linux sandboxing additionally requires `slirp4netns`, `nsenter`, `iptables`, `ip6tables`, `iptables-restore`, and `ip6tables-restore` on `PATH` — install them if sandboxed commands start failing to launch. Linux sandboxes also now restrict network egress to the configured proxy in proxy mode, which needs `slirp4netns`, `util-linux` 2.35+, `iptables`, and `/dev/net/tun` access.
