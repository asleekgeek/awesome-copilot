...
**Sorting the split Sessions sidebar** *(v1.0.83+)*: The sidebar now supports **Recent**, **Created**, **Name**, and the classic **None** sort orders, in addition to the existing picker sort options. Your selected order is remembered and restored the next time you restart the CLI.

**Windows 11 taskbar integration** *(v1.0.83+)*: On Windows 11, running Copilot sessions now surface in the taskbar with live hover status cards, so you can check on a background session's progress without switching back to its terminal window.
...
> **Disabling managed sandbox sessions from a bypass prompt (v1.0.84+)**: When an organization-managed sandbox policy blocks a command and you approve a bypass, you can now also choose to disable managed sandbox sessions for the rest of the current session directly from that approval prompt, instead of navigating to `/settings` separately.
