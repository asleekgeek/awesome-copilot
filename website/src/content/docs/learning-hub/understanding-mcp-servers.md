...
> **Windows authentication broker (WAM) support (v1.0.81+)**: On Windows, remote MCP servers protected by Microsoft Entra ID can now sign in through the OS authentication broker (Web Account Manager), usually with no prompt at all if you're already signed in to Windows with that account. Other platforms, `--device-code`, and Windows machines without the broker library keep the existing browser-based sign-in flow.
