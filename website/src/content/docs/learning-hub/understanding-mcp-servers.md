...
> **Client ID Metadata Document (CIMD) support** *(v1.0.83+)*: For MCP servers that support OAuth via a Client ID Metadata Document, Copilot CLI can authenticate using the server's published CIMD instead of requiring manual client registration. This simplifies OAuth setup for MCP servers that publish their client metadata at a well-known URL.
